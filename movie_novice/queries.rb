...
def below_average_years
  ...
  Movie.select("yr, COUNT(*) AS bad_movies").where("score < 5").group("yr").order("COUNT(yr) DESC")
end
...
def pulp_fiction_actors
  ...
  Actor.select("actors.id, actors.name").joins(:castings).uniq.joins(:movies).uniq.where("movies.title = 'Pulp Fiction'")
end
...
def uma_movies
  ...
  Movie.select("movies.id, movies.title, movies.yr").joins(:castings).uniq.joins(:actors).uniq.where("actors.name ='Uma Thurman'").order("movies.yr ASC")
end
